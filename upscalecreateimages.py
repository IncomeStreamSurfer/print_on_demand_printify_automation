...
from config import openai_key, stability_ai_key  # Import necessary variables from config.py


# Set OpenAI and Stability.ai API keys https://platform.openai.com/account/api-keys AND https://beta.dreamstudio.ai/accountopenai.api_key = 'your_key_here'
...
def upscale_create_images():  # Encapsulate the main functionality in a function
    ...
    for idx, row in tqdm(df.iterrows(), total=df.shape[0]):
        ...
        data = {
            "width": 1024,  # Upscale the image to 1024x1024
            "height": 1024,  # Upscale the image to 1024x1024
            "text_prompts": [
                {
                    "text": image_prompt,
                    "weight": 0.5
        ...
        file_name = f"upscaled_image_{idx}.png"
        upscaled_local_path = f"{file_name}"  # Save the image in the same directory
        image.save(upscaled_local_path)

        # Append the generated data to the lists
        file_names.append(file_name)
        upscaled_local_paths.append(upscaled_local_path)
        titles.append(title)
    ...
    output_df = pd.DataFrame({
        ...
        "upscaled_local_path": upscaled_local_paths,
        "title": titles,
    ...
    output_df.to_csv("upscaled_output.csv", index=False)
